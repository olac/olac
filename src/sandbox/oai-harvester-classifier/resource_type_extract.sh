...
select METADATA_ELEM.Item_ID,'',group_concat(replace(Content,'\\n',' ') SEPARATOR ' *** ') from ARCHIVED_ITEM inner join METADATA_ELEM on ARCHIVED_ITEM.Item_ID = METADATA_ELEM.Item_ID where (TagName = 'publisher' or TagName = 'description' or TagName = 'creator' or TagName = 'title' or TagName = 'contributor' or TagName = 'subject') and (TypeClassifiedDate is NULL or TypeClassifiedDate < ARCHIVED_ITEM.DateStamp) group by METADATA_ELEM.Item_ID;
"
else
SQL="set session group_concat_max_len = 10000;
