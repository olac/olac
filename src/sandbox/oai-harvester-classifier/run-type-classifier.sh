...
# "select MAX(ARCHIVE_ID) from ARCHIVED_ITEM"
NUM_ARCHIVES=487
...
do
   ...
   time ./reset_for_type_enrichment.sh $ARCHIVE

   # export oai data
   echo Exporting archive $ARCHIVE oai data to be classified...
   time ./resource_type_extract.sh $ARCHIVE > $TMPA 
   ...
   #time $UTF8CONDITIONER -q < $TMPA > $TMP0
