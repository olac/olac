...
sub get_earliestDatestamp {
	...
	("select min(DateStamp) from ARCHIVED_ITEM_FILTERED where DateStamp>'0000-00-00'")->[0];
}
...
sub getTable_GetRecord {
	...
	("select * from ARCHIVED_ITEM_FILTERED " .
...
sub getTable_ListIdentifiers {
    ...
    my $query = "select OaiIdentifier, DateStamp from ARCHIVED_ITEM_FILTERED ";
...
sub getTable_ListMetadataFormats {
    ...
    return $self->{dbh}->selectrow_arrayref
	("select Item_ID from ARCHIVED_ITEM_FILTERED " .
	 "where OaiIdentifier='$request->{identifier}'");
}
...
sub getTable_ListRecords {
    ...
    my ($f, $u, $a, $b);  # from, until, above, below
    my ($result,$order,$limit);

    #####
    # prepare temporary DB
    $query = "DROP TABLE IF EXISTS `ARCHIVED_ITEM_RANDOM`";
    $result = $self->{dbh}->do($query);

    $query = "CREATE TEMPORARY TABLE `ARCHIVED_ITEM_RANDOM` AS SELECT * FROM `ARCHIVED_ITEM_FILTERED` ";
   # probability range
    if ($request->{above} || $request->{below}) {
        # make sure they're numbers
        $a = $request->{above} =~ /^\d+(\.\d+)$/ ? " and me.Content >= ". $request->{above} : "";
        $b = $request->{below} =~ /^\d+(\.\d+)$/ ? " and me.Content <= ". $request->{below} : "";
#     left join EXTENSION ex on me.Extension_ID=ex.Extension_ID
#     left join CODE_DEFN cd on me.Extension_ID=cd.Extension_ID and me.Code=cd.Code
        $query .= "WHERE Item_ID = ANY
(select oa.Item_ID
from (ARCHIVED_ITEM_FILTERED oa join METADATA_ELEM me on oa.Item_ID=me.Item_ID)
    where me.Extension_ID = 15 
    and me.Type = 'binary' $a $b) ";

    }
    if ($request->{random}) {
       $query .= "ORDER BY RAND() ";
       $query .= "LIMIT ". $request->{random} ." ";
    }
    else {
       $query .= "ORDER BY RAND() ";
       $query .= "LIMIT 5000 ";
    ...
    #$query = "select OaiIdentifier, DateStamp, Item_ID from ARCHIVED_ITEM_FILTERED ";
    $query = "select OaiIdentifier, DateStamp, Item_ID from ARCHIVED_ITEM_RANDOM ";
...
from (ARCHIVED_ITEM_RANDOM oa join METADATA_ELEM me on oa.Item_ID=me.Item_ID)
     left join EXTENSION ex on me.Extension_ID=ex.Extension_ID
     left join CODE_DEFN cd on me.Extension_ID=cd.Extension_ID and me.Code=cd.Code ";
...
sub getTable_Query {
	...
	"from ARCHIVED_ITEM_FILTERED as a ";
