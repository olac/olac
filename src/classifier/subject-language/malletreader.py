"""
FIRST DRAFT -- this needs to be cleaned up and simplified,
since it really just reads 'Archive_ID\t\tDescription'

Corpus readers for the Tab-delineated table format used by
MALLET.  The corpus reader scans through the
...
table.  E.g.:
    ...
    >>> reader = MalletCorpusReader('oai_classifier_trn', '.*db\.tmp')
...
class MalletCorpusReader(CorpusReader):
    def records(self, fileids=None):
        if fileids is None: fileids = self._fileids
        elif isinstance(fileids, basestring): fileids = [fileids]
        return concat([MalletCorpusView(fileid,encoding=enc)
                       for (fileid, enc) in self.abspaths(fileids, True)])

class MalletCorpusView(StreamBackedCorpusView):
...
#                'Code\tContent\n')
        ...
        while True:
            ...
            if re.search('^Item_ID', line): continue
            #print line, '\n'
            fieldvals = line.rstrip('\n').split('\t')
            (item_id, empty, description) = fieldvals
            if not record:
