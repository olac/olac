...
# hack to allow restoration of old pickles
class iso639_classifier:
    def __init__(self, *args):
        if(len(args) == 1):
            pickleFile = args[0]
            former_self = self.__restore(pickleFile)
            self.tree = former_self.tree
            self.country_lang = former_self.country_lang
            print "RESTORED OLD FROM FILE"
        else:
        ...
        self.first_chars = u"‡!’/'"
        # stoplist of tokens that are too common
        self.stoplist = set(['the','some','central','western','eastern','northern','southern','north','south','east','west']) 
        self.gs = None # if it exists, this is a list of lines from the gold standard
        self.ending = "<<END>>"

    @classmethod
    def __restore(self,filename):
        '''Restores data from a binary pickle file.'''
        file = open(filename,'rb')
        self.__class__ = iso639_classifier
...
# real language classifier definition
class iso639Classifier:
    ...
    def classify_records(self, debug, records, outstream, threshold, snw, wnw, a, b):
        ...
        for record in records:
            ...
            i += 1
            iso_dict, NE_results = self.classify(bag_of_words, snw, wnw, a, b)
...
#                   iso_results.append(top_choice)
            ...
            print>>outstream, '\t'.join([record['Item_ID'], ' '.join(iso_results), title])
            if debug:
                if self.gs:
    ...
    def classify_record(self, record, threshold, snw, wnw, a, b, debug=False):
        ...
        descr = remove_diacritic(self.spaces.sub(' ',get_or_none(record, 'description')))
        bag_of_words = title + ' \\n ' + subject + ' \\n ' + descr # purposely adding a token here so that we won't get NEs recognized "over the borders"
        iso_results, NE_dict = self.classify(bag_of_words, snw, wnw, a, b)
        if debug:
            return iso_results
        else:
            return [i for i in iso_results if iso_results[i]>threshold]

    def classify(self, bag_of_words, snw, wnw, a, b):
    ...
    def classify_round_robin(self, bag_of_words):
        '''For a string of text, uses _identify to identify language, country
        and region names and creates sets of ISO 639 codes for the language,
        country and region names identified.   Uses weighted to determine
        weights for each iso code.
    ...
    def classify_records_round_robin(self, debug, records, file_header, params, thresholds):
        ...
        for record in records:
            ...
            subject = remove_diacritic(self.spaces.sub(' ',get_or_none(record, 'subject')))
            descr = remove_diacritic(self.spaces.sub(' ',get_or_none(record, 'description')))
            bag_of_words = title + ' \\n ' + subject + ' \\n ' + descr # purposly adding a token here so that we won't get NEs recognized "over the borders"
            gs_idx += 1
            iso_dicts, NE_results = self.classify_round_robin(bag_of_words)
