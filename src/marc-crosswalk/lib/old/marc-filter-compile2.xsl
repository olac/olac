...
<!-- filter-compile2.xsl
   Compile the stage 2 ("reject") filter for a MARC-to-OLAC crosswalk
   G. Simons, 4 Feb 2009
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:alias="AliasForXSLT"
   ...
   <xsl:template match="/marc-filter">
      <alias:stylesheet version="{$version}">
         ...
         <xsl:comment>If one of the following rejection criteria is met, then discard the
            record.</xsl:comment>
         <xsl:apply-templates select="reject-stage/*"/>
         <xsl:comment>If none of the reject criteria are met, then copy the record.</xsl:comment>
