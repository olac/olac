...
define('PENDING_DIR',   OLAC_PATH.'/register/pending'); # dir for pending hostless repos
define('HOSTLESS_DIR',  OLAC_PATH.'/devel/sr');         # dir for registered hostless repos
define('PENDING_URL',   OLAC_URL.'/register/pending');
define('HOSTLESS_URL',  OLAC_URL.'/devel/sr');
...
function postedurl_is_hostless() {
  ...
  global $POSTEDURL;
  if (preg_match("!^" . OLAC_URL . "/!", $POSTEDURL)) {
    return TRUE;
  } else {
    return FALSE;
  }
...
function sr_schema_valid($file) {
  ...
  exec(XSV." $file $xsv_xml_output", $arr, $retval);
  exec(XSLT." $xsv_xml_output ".XSV_XSL." $xsv_html_output", $arr, $retval);
  $output = get_output(XSLT." $xsv_xml_output ".XSV_TRANSFORM, CHAR_LIMIT);
  print str_replace("OK.", "XSV <font color=green>OK</font>", $output);
  if (substr_count($output, "OK") == 0 ) {
    $msg = @join(@file($xsv_xml_output),'');
...
function sr_schematron_valid($file) {
  ...
  $v1 = postedurl_is_hostless() || sr_baseurl_check($file);
...
function check_repository_status()
{
  ...
  if (is_null($rows[0]["dateApproved"])) {
    return "pending";
  } elseif (postedurl_is_hostless()) {
    if (is_hostless($rows[0]["BASEURL"])) {
      # the user might want to push a new hostless repository
