...
sub _fetch
{
    ...
    # fetch and parse
    my $http = new OLAC::HTTPStream($url) || return;
...
sub harvest {
    my $self = shift;
    foreach my $url (@{$self->{archive_list}}) {
	print "========================================\n";
	...
	my $from = next_day($lastdate) if $lastdate;
...
sub purge {
    my $self = shift;
    my $db = $self->{db};
    my $arc_list = $self->{archive_list};

    my $res = $db->{dbh}->selectcol_arrayref
	("select BaseURL from OLAC_ARCHIVE");
    my %h;
    map {$h{$_}=1;} @$res;
    map {delete $h{$_};} @$arc_list;
    foreach my $url (keys %h) {
	print $url . "\n";
	my $aids = $db->{dbh}->selectcol_arrayref
	    ("select Archive_ID from OLAC_ARCHIVE where BaseURL='$url'");
	foreach my $aid (@$aids) {
