...
objective 2 of hidden resources, and objective 3 of missing resources.
...
\citep{SimonsEtal04lex,SimonsEtal04text};
(vi) CD-ROM publications of linguistic field data:
...
% \todo{Gary, Mark: list some recent projects and publications}
...
metadata ctaloging and the OAI protocol for
...
\citep{Langendoen02,Lewis03}.  Another problem is \emph{volatility}:
...
results which are not currently returned in OLAC searches.
...
language documentation material (Still in the possession of the linguists who
...
Archival Information Systems (OAIS) reference model \citep{OAIS02} is
an ISO standard (ISO 14721:2003) that specifies best practices for
digital archiving.  It defines standards for the six basic functions
...
product at SourceForge where it is supported by a thriving developer
community that ranks in the 98th percentile for level of activity when
compared to all other projects
(\url{http://sourceforge.net/projects/dspace/}).  It is now in use at
more than one hundred universities as the institutional repository
where faculty members log in to deposit the fruits of their research
...
characterises a language resource; this is especially critical for
